...
else
{
    ...
    $sql = "INSERT INTO words (word)
            SELECT '" . $_GET['word'] . "' FROM dual
            WHERE NOT EXISTS (SELECT 1 FROM words WHERE word = '" . $_GET['word'] . "');";
    ...
    $sql = "INSERT INTO users_has_words (user_id, word_id)
            SELECT " . $user_id . ", " . $word_id . " FROM dual
            WHERE NOT EXISTS (SELECT 1 FROM users_has_words 
                ...
                <div class="col-md-8 flex-item">
                    <?php
                        if (isset($error) && !empty($error)) {
                            ...
                            echo "<h1>Add Confirmation</h1>";
                            echo "<p>You've added '" . $_GET['word'] . "' to My Words.</p>";
                        }
                    ?>
                </div>
            </div>
